...
public class ActionController {
	...
	public ResponseEntity<?> login(@RequestBody LoginRequest request) {
		...
		UserModel user = userCacheService.getByUsername(username).orElseGet(() -> {
			...
			userCacheService.cacheUserObject(saved);
			return saved;
		});

		// Create and send asynchronous MQ event for mission settlement
		rocketMQTemplate.convertAndSend("MISSION_TOPIC:LOGIN", new UserLoginEvent(user.getId(), LocalDate.now()));

		// Generate Token and store in Redis (expires in 24 hours)
		String token = UUID.randomUUID().toString();
		redis.set(redis.authTokenKey(token), String.valueOf(user.getId()), Duration.ofHours(24));
	...
	public ResponseEntity<?> launchGame(
		...
		// Generate Play Token and bind userId + gameId in Redis (expires in 30 minutes)
		String playToken = UUID.randomUUID().toString();
		redis.set(redis.playSessionKey(playToken), userId + ":" + gameId, Duration.ofMinutes(30));

		rocketMQTemplate.convertAndSend("MISSION_TOPIC:LAUNCH", new GameLaunchEvent(userId, gameId, LocalDate.now()));
	...
	public ResponseEntity<?> play(
		...
		String sessionKey = redis.playSessionKey(playToken);
		String sessionValue = redis.get(sessionKey);

		if (sessionValue == null) {
			return ResponseEntity.status(401).body(Map.of("error", "Unauthorized: Play session expired or invalid"));
		}

		// Parse userId and gameId from session value
		String[] parts = sessionValue.split(":");
		Long sessionUserId = Long.parseLong(parts[0]);
		Long gameId = Long.parseLong(parts[1]);

		// Ensure the caller is the same person who launched the game
		if (!authUserId.equals(sessionUserId)) {
			return ResponseEntity.status(403).body(Map.of("error", "Forbidden: Session owner mismatch"));
		}

		// Extend session expiration (another 30 minutes)
		redis.expire(sessionKey, Duration.ofMinutes(30));

		// Backend generates random score (50 ~ 500)
		int score = new java.util.Random().nextInt(451) + 50;

		gamePlayRecordRepository.save(GamePlayRecordModel.builder()
				.userId(authUserId)
				.gameId(gameId)
				.score(score)
				.build());

		rocketMQTemplate.convertAndSend("MISSION_TOPIC:PLAY", new GamePlayEvent(authUserId, gameId, score, LocalDate.now()));
